...
if ( ! function_exists( '<%= theme.slug %>_setup' ) ) :
	...
	function <%= theme.slug %>_setup() {
		...
		add_theme_support( 'custom-background', apply_filters( '<%= theme.slug %>_custom_background_args', array(
 ...
 */
function <%= theme.slug %>_content_width() {
	$GLOBALS['content_width'] = apply_filters( '<%= theme.slug %>_content_width', 640 );
}
add_action( 'after_setup_theme', '<%= theme.slug %>_content_width', 0 );
...
function <%= theme.slug %>_scripts() {
	wp_enqueue_style( '<%= theme.slug %>-style', get_stylesheet_uri(), array(), '<%= theme.version %>' );
